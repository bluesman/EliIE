...
def txt2conll(text_line, label_sign):
    ...
    # is the raw text without the annotation, only need to return one term list
    if label_sign == 0:
        term_list = text_line.split()
        return(term_list)

    # is the labelled text, need to return two lists: term list & label list
    if label_sign == 1:
...
# from tagged_text to raw_text
def clean_txt(tagged_text):
